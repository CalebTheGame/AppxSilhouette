...
public class PPSSilhouetteEstimtor extends Evaluator {
	...
	public double evaluate(Dataset<?> dataset) {
		...
		Broadcast<Double> s0 = sc.broadcast(Double.valueOf(Math.log(2*k/delta)));
		...
		coll2.foreach((ForeachFunction<MyPair>) pivot ->{
			...
			Iterator<SamplePair> initsampl = initsam.value()[clust].iterator();
			int ct = 0;
			double pps = 1/clustSizes.value()[clust];
			while (initsampl.hasNext()) {
				pps = Math.max(Math.sqrt(Vectors.sqdist(pivot.getFeats(), initsampl.next().getFeats()))/(distances.value()[clust] [ct]), pps);
				ct = ct++;
			}

			pps = Math.min(1.0, (dataInfo.value()[1]*pps));

			if (Math.random() < pps)
				samples.add(new SamplePair(pivot.getFeats(), pps), clust);
	...
	private class DistAccumulator extends AccumulatorV2<double[][], double[][]>{

		double[][] dists;

		public DistAccumulator(double[][] values) {
			super();
			this.dists = values;
		}

		public DistAccumulator(int clusters, int size) {
			super();
			this.dists = new double[clusters][size];
		}
		...
		public void add(double[][] v) {
			for (int i = 0; i < v.length; i++) {
				for (int j = 0; j < v[i].length; j++) {
					dists[i][j] = dists[i][j]+v[i][j];		
				}
			}
		}

		public void add(double v, int clust, int index) {
			dists[clust][index] = dists[clust][index] + v;
		}

		@Override
		public AccumulatorV2<double[][], double[][]> copy() {
			return new DistAccumulator(dists);
		}
		...
		@Override
		public void reset() {
			dists = new double[dists.length][dists[0].length];			
		}
