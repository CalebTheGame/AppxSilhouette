...
import altierif.appxSilhEval.FrahlerSolherSilhouette;
import altierif.appxSilhEval.PPSSilhouetteEstimtor;
import altierif.appxSilhEval.SimplifiedSilhouetteEvaluator;
import altierif.appxSilhEval.UniformSamplingClusteringEvaluator;
import altierif.exactSilhouette.ExactSilhouetteEvaluator;
...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		if ((Utils.index(args, "performanceSimplified")) != -1) {
			simplifiedSilh(args, spark);
			System.out.println("Done!");
			return;
		}
		
		if ((Utils.index(args, "performanceFralhSolh")) != -1) {
			fralhSolhSilh(args, spark);
			System.out.println("Done!");
			return;
		}
	...
	private static void exactSilhOnly(String[] args, SparkSession spark) {
		...
		double entrysize = Long.valueOf(SizeEstimator.estimate(loaded.first())).doubleValue();
		...
		double fract = (spark.sparkContext().conf().getSizeAsBytes("spark.executor.memory") *
				...
				0.6 * 0.3 // algo safe zone
				) / (Integer.parseInt(spark.conf().get("spark.executor.cores")) * 1.0);
		System.out.println(fract);

		ExactSilhouetteEvaluator naiveEval = new ExactSilhouetteEvaluator();
		if (parts != 0) {
			naiveEval.setPartitions(parts);
		} else {
			naiveEval.setPartitions(Math.max(Integer.parseInt(spark.conf().get("spark.executor.cores")),
					(int) (Math.ceil(Math.ceil(totsizeBytes / fract)
					/ (Integer.parseInt(spark.conf().get("spark.executor.instances")) * 1.0)))
					* Integer.parseInt(spark.conf().get("spark.executor.instances"))));
		}
		int bufsize = (Math.max(Long.valueOf(loaded.count()).intValue(), (int) (Math.ceil(Math.ceil(fract / entrysize)))));
		System.out.println("bufsize: "+bufsize);
		naiveEval.setBufsize(bufsize);
	...
	private static void PPSSamplingAppx(String[] args, SparkSession spark) {
		...
		if (parts != 0) {
			...
			System.out.println(fract);
			appxEval.setPartitions(Math.max(Integer.parseInt(spark.conf().get("spark.executor.cores")),
					(int) (Math.ceil(Math.ceil(totsizeBytes / fract)
					/ (Integer.parseInt(spark.conf().get("spark.executor.instances")) * 1.0)))
					* Integer.parseInt(spark.conf().get("spark.executor.instances"))));
		}
	...
	private static void uniformSamplingAppx(String[] args, SparkSession spark) {
		...
		UniformSamplingClusteringEvaluator appxEval = new UniformSamplingClusteringEvaluator(t);
		if (parts != 0) {
			appxEval.setPartitions(parts);
		} else {
			double totsizeBytes = Long.valueOf(SizeEstimator.estimate(loaded.first())).doubleValue() * (loaded.count());
		...
		csvStats.writeLn("Uniform," + loaded.count() + "," + loaded.groupBy("prediction").count().collectAsList().size()
				+ "," + appxSilhouette + "," + overallTime);
	...
	private static void simplifiedSilh(String[] args, SparkSession spark) {
		// ...
		Dataset<Row> loaded;
		...
		SimplifiedSilhouetteEvaluator simplSilh = new SimplifiedSilhouetteEvaluator();
		if (parts != 0) {
			simplSilh.setPartitions(parts);
		} else {
			double totsizeBytes = Long.valueOf(SizeEstimator.estimate(loaded.first())).doubleValue() * (loaded.count());
			System.out.println(totsizeBytes);
			double fract = (spark.sparkContext().conf().getSizeAsBytes("spark.executor.memory") *
					...
					0.2 *
					// Double.parseDouble(spark.sparkContext().conf().get("spark.shuffle.safetyFraction"))
					0.6 * 0.6 // algo safe zone
					) / (Integer.parseInt(spark.conf().get("spark.executor.cores")) * 1.0);
			System.out.println(fract);
			simplSilh.setPartitions(Math.max(Integer.parseInt(spark.conf().get("spark.executor.cores")),
					(int) (Math.ceil(Math.ceil(totsizeBytes / fract)
					/ (Integer.parseInt(spark.conf().get("spark.executor.instances")) * 1.0)))
					* Integer.parseInt(spark.conf().get("spark.executor.instances"))));
		}
		long startTimeNaive = System.nanoTime();
		double silhouette = simplSilh.evaluate(loaded);
		long endTimeNaive = System.nanoTime();
		double overallTime = (endTimeNaive - startTimeNaive) / 10e8;
		csvStats.writeLn("Simplified," + loaded.count() + "," + loaded.groupBy("prediction").count().collectAsList().size()
				+ "," + silhouette + "," + overallTime);
	}
	
	private static void fralhSolhSilh(String[] args, SparkSession spark) {
		// ...
		Dataset<Row> loaded;
		...
		FrahlingSolherSilhouette fraSollSilh = new FrahlingSolherSilhouette();
		if (parts != 0) {
			fraSollSilh.setPartitions(parts);
		} else {
			double totsizeBytes = Long.valueOf(SizeEstimator.estimate(loaded.first())).doubleValue() * (loaded.count());
			System.out.println(totsizeBytes);
			double fract = (spark.sparkContext().conf().getSizeAsBytes("spark.executor.memory") *
					...
					0.2 *
					// Double.parseDouble(spark.sparkContext().conf().get("spark.shuffle.safetyFraction"))
					0.6 * 0.1 // algo safe zone
					) / (Integer.parseInt(spark.conf().get("spark.executor.cores")) * 1.0);
			System.out.println(fract);
			fraSollSilh.setPartitions(Math.max(Integer.parseInt(spark.conf().get("spark.executor.cores")),
					(int) (Math.ceil(Math.ceil(totsizeBytes / fract)
					/ (Integer.parseInt(spark.conf().get("spark.executor.instances")) * 1.0)))
					* Integer.parseInt(spark.conf().get("spark.executor.instances"))));
		}
		long startTimeNaive = System.nanoTime();
		double silhouette = fraSollSilh.evaluate(loaded);
		long endTimeNaive = System.nanoTime();
		double overallTime = (endTimeNaive - startTimeNaive) / 10e8;
		csvStats.writeLn("FralhSolh," + loaded.count() + "," + loaded.groupBy("prediction").count().collectAsList().size()
				+ "," + silhouette + "," + overallTime);
	}
