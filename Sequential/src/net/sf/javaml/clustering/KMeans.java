 ...
 * Copyright (c) 2019, Federico Altieri
 * 
...
public class KMeans implements Clusterer {
	...
	/**
	 * Sets the seed for the random generators used in the KMeans algorithm
	 * @param seed 
	 */
	...
	/**
	 * @return an array with the centroids computed with the last call of "cluster(Dataset)" method
	 */
	public Instance[] getCentroids() {
		return centroids;
	}

	public static double getOverallKMeansCostFunction(Dataset[] clusters , DistanceMeasure dm) {
		double score = 0.0;
		Instance[] centroids = new Instance[clusters.length];
		int instanceLength = clusters[0].instance(0).noAttributes();

		// When all objects have been assigned, recalculate the positions of
		// the K centroids and start over.
		// The new position of the centroid is the weighted center of the
		// current cluster.
		double[][] sumPosition = new double[clusters.length][instanceLength];
		int[] countPosition = new int[clusters.length];
		for (int k = 0; k < clusters.length; k++) {
			for (int i = 0; i < clusters[k].size(); i++) {
				Instance in = clusters[k].instance(i);
				for (int j = 0; j < instanceLength; j++) {

					sumPosition[k][j] += in.value(j);
		...
		for (int i = 0; i < clusters.length; i++) {
			...
			centroids[i] = new DenseInstance(tmp);
		}

		for (int k = 0; k < clusters.length; k++) {
			for (int i = 0; i < clusters[k].size(); i++) {
				score = score + Math.pow((dm.measure(clusters[k].instance(i), centroids[k])), 2);
			}
		}
		return score;
	}
